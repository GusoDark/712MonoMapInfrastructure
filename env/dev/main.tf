module "dev_vm" {
  ...
  domain                   = var.DOMAIN
  resource_group           = "MN_RG_TeamVii"  # Modificado
  nic_name                 = "MN_NIC_TeamVii"  # Modificado
  mail_service             = var.MAIL_SERVICE
  security_group_name      = "MN_SG_TeamVii"   # Modificado
  ssh_key_path             = "./keys/712monomap_server"
  port                     = var.PORT
  server_name              = "MN_Server_TeamVii"  # Modificado
  location                 = "eastus2"
  mapbox_access_token      = var.MAPBOX_ACCESS_TOKEN
  mongo_url                = var.MONGO_URL
  subnet_name              = "MN_SUBNET_TeamVii"  # Modificado
