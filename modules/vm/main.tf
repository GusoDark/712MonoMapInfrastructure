...
resource "azurerm_resource_group" "MN_RG" {
...
resource "azurerm_virtual_network" "MN_VNET" {
  name                = "${var.vnet_name}-${var.environment}"
  resource_group_name = azurerm_resource_group.MN_RG.name
  location            = var.location
...
#Construir Subnet
resource "azurerm_subnet" "MN_SUBNET" {
  name                 = "${var.subnet_name}-${var.environment}"
  resource_group_name  = azurerm_resource_group.MN_RG.name
  virtual_network_name = azurerm_virtual_network.MN_VNET.name
  address_prefixes     = ["10.123.1.0/24"]
}

# Construir security groups
resource "azurerm_network_security_group" "MN_SG" {
...
resource "azurerm_subnet_network_security_group_association" "MN-SGA" {
  subnet_id                 = azurerm_subnet.MN_SUBNET.id
  network_security_group_id = azurerm_network_security_group.MN_SG.id
}

# Crear IP Publica
resource "azurerm_public_ip" "MN-IP" {
...
resource "azurerm_network_interface" "MN-NIC" {
  name                = var.nic_name
  location            = var.location
  resource_group_name = azurerm_resource_group.MN_RG.name
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.MN_SUBNET.id
    public_ip_address_id          = azurerm_public_ip.MN-IP.id
...
resource "azurerm_linux_virtual_machine" "MN-VM" {
  name                  = "${var.server_name}-${var.environment}"
  resource_group_name   = azurerm_resource_group.MN_RG.name
  location              = var.location
  size                  = "Standard_B2s"
  admin_username        = var.admin_username
  network_interface_ids = [azurerm_network_interface.MN-NIC.id]
...
resource "time_sleep" "wait_2_minutes" {
  depends_on      = [azurerm_linux_virtual_machine.MN-VM]
...
resource "null_resource" "init_docker" {
  ...
  connection {
    ...
    user        = var.admin_username
    private_key = file(var.ssh_key_path)
    host        = azurerm_linux_virtual_machine.MN-VM.public_ip_address
  }
  provisioner "remote-exec" {
